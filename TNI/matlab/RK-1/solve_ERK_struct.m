function [tvals,Y,nsteps,output] = solve_ERK_struct(fcn,tvals,Y0,par)
% usage: [tvals,Y,nsteps,output] = solve_ERK_struct(fcn,tvals,Y0,B,par)
%
...
dt_a = 0;       % number of accuracy-limited time steps
...
for tstep = 2:length(tvals)
   ...
   while ((t-tvals(tstep))*dt < 0)
      ...
      if (adaptive)
            [Ynew,Yerr] = ERKstep_embedded(fcn, Y0, t, dt, B);
            nsteps = nsteps + 1;
      else
         [Ynew] = ERKstep_basic(fcn, Y0, t, dt, B);
         nsteps = nsteps + 1;
      end
...
output = struct('dtvals',dtstore,'nfail',a_fails);
