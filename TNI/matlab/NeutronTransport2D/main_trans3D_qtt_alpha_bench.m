...
ttiV = 0; % velocity term for alpha-eigenvalue problem
for kxi = 1:2 % 1-minus, 2-plus
  for jeta = 1:2
    for imu = 1:2
      ...
      %velocity term - very similar to the interpolation term
      iVtemp = matrices_to_qtt_matrix_fn({intp{imu}, intp{jeta}, intp{kxi},...
        angmat, diag(1./vel)}, tt_tol);
...
% solve
[alphatt, ttPsi1, Itertime, convrate, lambda, k_num_iter] = tt_SolveEig_Alpha(ttH, ttS, ttF, ttiV, params,...
  mv_opts, amen_solve_opts);
%% PRINT THE RESULTS
true_alpha = 9.2410746e-02;
fprintf('eigenvalue k = %.10f \n', alphatt)
fprintf('eigenvalue errors = %.5e \n', norm(alphatt-true_alpha));
...
fprintf('compression ratio of V = %.5e \n', compress_ratio_tt(ttiV));
fprintf('Elapsed time = %.2f seconds \n',sum(Itertime))

%%
filename = sprintf('./Results/alpha_problem_%d.mat',n);
save(filename, ...
  'alphatt','ttPsi1','ttH','ttS','ttF','ttiV','Itertime','true_alpha','params','mv_opts',...
  'amen_solve_opts','convrate','lambda','k_num_iter');
