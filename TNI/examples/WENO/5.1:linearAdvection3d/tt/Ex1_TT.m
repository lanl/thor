%% Info
% Example 5.1 : 3D Linear Advection in tensor-train format
% Developer   : M. Engin Danis
% Notes       :
% It is recommended to run this script on terminal:
% (1) do: export TTWENOLIB=/path/of/WENO-paper
% (2) do: matlab -nodesktop -nojvm -nosplash -nodisplay -singleCompThread -r "try, run('Ex1_TT.m'); catch err, disp(getReport(err)); end;exit;"
...
CFL       = 1;                                    % CFL Number for adaptive time-stepping
...
funFlux   = @F_linearWave;                        % Function handle for flux vector computation in each direction 
funEig    = @Eig_linearWave;                      % Function handle for computing eigenvalues of flux jacobian    
...
C_eps     = 500;                                  % Empricial coefficient in Eq. (35) of the TT-WENO paper by Danis et al. 
...
%addpath(fdwenolib.tt+"/equations/LinearWave3d");
