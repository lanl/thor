%% Info
% Example 5.5 : 3D Burgers' equation in tensor-train format with a shock
% Developer   : M. Engin Danis
% Notes       :
% It is recommended to run this script on terminal:
% (1) do: export TTWENOLIB=/path/of/WENO-paper
% (2) do: matlab -nodesktop -nojvm -nosplash -nodisplay -singleCompThread -r "try, run('Ex5_TT.m'); catch err, disp(getReport(err)); end;exit;"
% This script writes 1d results (x, u_tt) for each grid level under "results" directory, which can be used for postprocessing if needed.
clear; clc; close all;
...
C_eps     = 100;                                  % Empricial coefficient in Eq. (35) of the TT-WENO paper by Danis et al. 
...
%addpath(fdwenolib.tt+"/equations/Burgers3d");
