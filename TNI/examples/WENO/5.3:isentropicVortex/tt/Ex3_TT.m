%% Info
% Example 5.3 : Advection of an isentropic vortex in tensor-train format
% Developer   : M. Engin Danis
% Notes       :
% It is recommended to run this script on terminal:
% (1) do: export TTWENOLIB=/path/of/WENO-paper
% (2) do: matlab -nodesktop -nojvm -nosplash -nodisplay -singleCompThread -r "try, run('Ex3_TT.m'); catch err, disp(getReport(err)); end;exit;"
clear; clc; close all;
...
Tfinal    = 1;                                    % Final Time of the simulation   
...
funBC     = @BCisentropicVortex;                  % Function handle for boundary conditions                       
...
C_eps     = 2/sqrt(Lx*Ly*Lz);                     % Empricial coefficient in Eq. (35) of the TT-WENO paper by Danis et al. 
