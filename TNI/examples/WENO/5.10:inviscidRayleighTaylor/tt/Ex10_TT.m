%% Info
% Example 5.10 : Inviscid Rayleigh-Taylor problem iin tensor-train format
% Developer    : M. Engin Danis
% Notes        :
% It is recommended to run this script on terminal:
% (1) do: export TTWENOLIB=/path/of/WENO-paper
% (2) do: matlab -nodesktop -nojvm -nosplash -nodisplay -singleCompThread -r "try, run('Ex10_TT.m'); catch err, disp(getReport(err)); end;exit;"
...
addpath(genpath('../../../../matlab/WENO/src/tt/'))
...
%% Input parameters to ttFDWenoSolver.m (Nx, Ny, Nz will be set later)
Tfinal    = 1.95;                                  % Final Time of the simulation   
dtOption  = 1;                                     % Time Stepping Option           
CFL       = 0.5;                                   % CFL Number for adaptive time-stepping
Neq       = 5;                                     % Number of equations 
N1d       = 30*(2.^linspace(1,num_lvls,num_lvls)); % Number of cells in each direction
Lx        = 0.25;                                  % Domain length in the x-direction 
Ly        = 1;                                     % Domain length in the y-direction 
Lz        = 0.25;                                  % Domain length in the z-direction 
...
gam       = 5/3;                                   % Ratio of specific heats (not used here, so it is left empty)
eps_tt    = 1e-14;                                 % Initial tolerance for tt trunctation                                   
eps_cross = 1e-14;                                 % Initial tolerance for tt cross interpolation                           
C_eps     = 1e5;                                   % Empricial coefficient in Eq. (35) of the TT-WENO paper by Danis et al. 
...
%% Setup library paths
%restoredefaultpath;
%run(getenv("TTWENOLIB")+"/tt_setup.m");
%% Setup equation specific path for flux and eigenvalue routines 
%addpath(fdwenolib.tt+"/equations/Euler3d");
...
for lvl=1:length(N1d)
    ...
    fprintf("%s",repelem("-",70));fprintf("\n");
    %
    data = ttFDWenoSolver(Tfinal, dtOption, CFL, Neq, Nx, Ny, Nz, Lx, Ly, Lz,...
                          funFlux, funEig, funInit, funBC, isSource, gam, ...
