%% Info
% Example E   : Stable Barotropic Jet in full-tensor format
% Developer   : M. Engin Danis
% Notes       :
% It is recommended to run this script on terminal:
% (1) do: export TTSWELIB=/path/of/SWE-paper
% (2) do: matlab -nodesktop -nojvm -nosplash -nodisplay -singleCompThread -r "try, run('ExE_FT.m'); catch err, disp(getReport(err)); end;exit;"
...
%% Input parameters to ftFVSWEsolver.m (Nx, Ny, CFL, dtOption, Recon will be set later)
...
for recon_idx = 1:length(ReconList)
    ...
    for lvl=1:length(N1d)
        ...
        Qe = data.Qe;  % exact solution (this is actually the same as the initial condition)
        %
        % Dimensionalize Qe
        %
