%% Info
% Example 5.9 : Double Mach reflection problem in full-tensor format
% Developer   : M. Engin Danis
% Notes       :
% It is recommended to run this script on terminal:
% (1) do: export TTWENOLIB=/path/of/WENO-paper
% (2) do: matlab -nodesktop -nojvm -nosplash -nodisplay -singleCompThread -r "try, run('Ex9_FT.m'); catch err, disp(getReport(err)); end;exit;"
...
for lvl=1:length(N1d)
    ...
    Nx = N1d(lvl);
    Ny = Nx/4; 
    Nz = 1;  % change this to Nx/4 for a full 3d simulation
    %
