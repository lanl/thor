%% Info
% Example 5.9 : Double Mach reflection problem in tensor-train format
% Developer   : M. Engin Danis
% Notes       :
% It is recommended to run this script on terminal:
% (1) do: export TTWENOLIB=/path/of/WENO-paper
% (2) do: matlab -nodesktop -nojvm -nosplash -nodisplay -singleCompThread -r "try, run('Ex9_TT.m'); catch err, disp(getReport(err)); end;exit;"
...
N1d       = 240*(2.^linspace(1,num_lvls,num_lvls));% Number of cells in each direction
Lx        = 4;                                     % Domain length in the x-direction 
...
C_eps     = 1e8;                                   % Empricial coefficient in Eq. (35) of the TT-WENO paper by Danis et al. 
