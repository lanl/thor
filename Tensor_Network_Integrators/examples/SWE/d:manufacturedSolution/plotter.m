...
function plotter_error(fig_path)
    ...
    for eq=1:1
        ...
        xlim([N(1) N(end)]);
        xticks(N);
        yticks(sort(10.^[-8:2:0]));
        %
        annotation("textbox",[0.5 0.345 0.0 0.0],"String","$3^{rd}$ order","FitBoxToText","on","EdgeColor","none","FontSize",fs,"Interpreter","latex");
        annotation("textbox",[0.5 0.285 0.0 0.0],"String","$4^{th}$ order","FitBoxToText","on","EdgeColor","none","FontSize",fs,"Interpreter","latex");
        annotation("textbox",[0.5 0.22  0.0 0.0],"String","$5^{th}$ order","FitBoxToText","on","EdgeColor","none","FontSize",fs,"Interpreter","latex");
        %
        set(gca,"FontSize",fs);
        %
...
function plotter_speed(fig_path)
    ...
    set(gca,"YScale","log");
    %
    set(gca,"XLim",[40 1280]);
    set(gca,"XTick",20*2.^(1:6));
