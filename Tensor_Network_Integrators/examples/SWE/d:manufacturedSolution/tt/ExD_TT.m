%% Info
% Example D   : Nonlinear manufactured solution in tensor-train format
% Developer   : M. Engin Danis
% Notes       :
% It is recommended to run this script on terminal:
% (1) do: export TTSWELIB=/path/of/SWE-paper
% (2) do: matlab -nodesktop -nojvm -nosplash -nodisplay -singleCompThread -r "try, run('ExD_TT.m'); catch err, disp(getReport(err)); end;exit;"
...
Ref.U      = 0.01;                                  % Reference velocity scale
Ref.H      = 500;                                   % Reference depth scale
isManuf    = true;                                  % Flag to use user-defined "manuf.m" to handle manufactured solutions terms
...
for recon_idx = 1:length(ReconList)
    ...
    for lvl=1:length(N1d)
        ...
        Qe = solExact(data.tt.q_xy{1},data.tt.q_xy{2},Tfinal/data.ref.t,data); % exact solution
        %
