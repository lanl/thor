...
% isQtt            : flag to manage QTT conversion                        %
% ------------------------------------------------------------------------%
function tt = opinf(x,u,t,K,isA,isF,tint_order,dt,dtp,tp,gamma,eps_tt,isQtt)
  %
  tic;
  %
  if(~exist("isQtt","var"))
    isQtt = true;
  elseif(~islogical(isQtt))
    error("isQtt must be a logical: true or false")
  end
  %
  if(ndims(x)~=3)
    error("The snapshots must be a 3-d tensor or TT: N x Neq x Nt");
  end
  %
  n = size(x);
  %
  if(isa(x,'tt_tensor'))
    x       = tt_reshape(x,[factor(n(1)) n(2) n(3)]);
    tt.Tpod = 0; % snapshots already calculated in tt format
  else
    if(isQtt)
      x = reshape(x,[factor(n(1)) n(2) n(3)]);
    end
    t1      = toc;
  ...
  %
  t1=toc;
  [A,F,B,D,R] = inferOp(x,j1,j2,U,tint_order,dt,isA,isF,gamma,eps_tt,isQtt);
  t2=toc;
  %
  % perform prediction
  %
  ...
  G  = core2cell(x);
  G{end} = G{end}(:,j1-1,:);
  G{end-1} = tensorprod(G{end-1},G{end},3,1);
  x  = cell2core(tt_tensor,G(1:end-1));
  xp = predict(x,u,A,F,B,tp,dtp,tint_order,isA,isF,eps_tt);
  t4 = toc;
  %
  % prepare output
  %
  tt.j1       = j1;
  tt.j2       = j2;
  tt.xp       = tt_reshape(tt_reshape(xp,[prod(n(1:end-1)),1]),n(1:end-1));
  tt.ranks    = xp.r(2:end-1);
  tt.dims     = xp.d;
  tt.A        = A;
  tt.F        = F;
  tt.B        = B;
