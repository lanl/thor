...
for kxi = 1:2 % 1-minus, 2-plus
  for jeta = 1:2
    for imu = 1:2
      ...
      %velocity term
      iVtemp = ttt_out_prod_fn({intp{imu}, intp{jeta}, intp{kxi},...
        angmat, diag(1./vel)});
...
iVmat = get_mat_10D_fn(iV,nx,ny,nz,nords,nE);

%% solve alpha - eigenvalue problem
A = iVmat\(Smat+Fmat-Hmat);
...
ttiV = convert_tensor_op_to_tt_matrix_fn(iV,10);
