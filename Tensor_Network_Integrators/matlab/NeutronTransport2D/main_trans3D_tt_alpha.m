...
vel = Param.vel;
%%%%%% 
nusigf = 3*Param.nusigf; %multiply by 3 to make alpha positive
%%%%%
nE = length( sigt );
chi    = ones( nE,1 )/nE;
nusigf = chi.*nusigf';
...
for kxi = 1:2 % 1-minus, 2-plus
  for jeta = 1:2
    for imu = 1:2
      ...
      iVtemp = matrices_to_tt_matrix_fn({intp{imu}, intp{jeta}, intp{kxi},...
        angmat, diag(1./vel)});
...
% solve
[ttalpha, ttPsi1, Itertime, convrate, lambda, k_num_iter] = tt_SolveEig_Alpha(ttH, ttS, ttF, ttiV, params,...
  mv_opts, amen_solve_opts);

%% alpha-eigenvalue

fprintf('TT alpha = %.10f \n', ttalpha(end))
