function [tvals,Ytt,nsteps,output] = solve_ERKtt_struct(fcn,tvals,Y0,par)
% usage: [tvals,Ytt,nsteps,output] = solve_ERKtt_struct(fcn,tvals,Y0,par)
%
...
for tstep = 2:length(tvals)
   ...
   while ((t-tvals(tstep))*dt < 0)
      ...
      if (st_fail == 0)
         ...
         % Experiment with adaptive ett, 1e-14 < ett < rtol
         % ett = max(dt^p,1e-14);
         % ett = min(ett,rtol);
...
output = struct('dtvals',dtstore,'nfail',a_fails,'ettvals',ettstore,'rank',rank);
