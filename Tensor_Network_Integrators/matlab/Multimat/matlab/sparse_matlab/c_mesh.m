classdef c_mesh < handle
  ...
  properties
    ...
    mixcell_for_3dcell = [];  % 3D cell to the index of
  ...
  methods
    ...
    function set_mesh(obj, dim, xl, xr, ncell, nbdry, nmat, matids)
      ...
      if dim == 2
        ...
        obj.cs_for_2dcell = zeros(ncell_ext(2), ncell_ext(1));
      ...
      elseif dim == 3
        ...
        obj.cs_for_3dcell = zeros(ncell_ext(3), ncell_ext(2), ncell_ext(1));
    ...
    function nmix_cell = set_mesh_mat(obj, dim, btype_lower, btype_upper, ...
      ...
      if dim == 2
        obj.get_node_vel_2d();
        bdry.bdry_node_2d(obj,btype_lower,btype_upper);

      elseif dim == 3
        obj.get_node_vel_3d();
        bdry.bdry_node_3d(obj,btype_lower,btype_upper);
      end
    ...
    function mesh_sound_speed(obj, nmat, matids, is_solid, gamma_ea_mat)
      ...
      % Determine the highest material ID and map gamma values to material IDs
      matid = max(obj.matid_for_2dcell(:));  % Assuming same matids for 2D and 3D
      ...
      if dim == 2
        for j = 1:ncell_ext(2)
          for i = 1:ncell_ext(1)
            nm = obj.nmat_for_2dcell(j, i);
            if nm ~= 1, continue; end

            matid = obj.matid_for_2dcell(j, i);
            if ~is_solid(matid)
              obj.cs_for_2dcell(j, i) = sqrt(gamma_matid(matid) * obj.pres_for_2dcell(j, i) / ...
                (obj.rho_for_2dcell(j, i) + tiny));
            else
              obj.cs_for_2dcell(j, i) = eos.sound_speed_solid(obj.rho_for_2dcell(j, i));
            end
          end
        end
      ...
      elseif dim == 3
        for k = 1:ncell_ext(3)
          for j = 1:ncell_ext(2)
            for i = 1:ncell_ext(1)
              nm = obj.nmat_for_3dcell(k, j, i);
              if nm ~= 1, continue; end

              matid = obj.matid_for_3dcell(k, j, i);
              if ~is_solid(matid)
                obj.cs_for_3dcell(k, j, i) = sqrt(gamma_matid(matid) * obj.pres_for_3dcell(k, j, i) / ...
                  (obj.rho_for_3dcell(k, j, i) + tiny));
              else
                obj.cs_for_3dcell(k, j, i) = eos.sound_speed_solid(obj.rho_for_3dcell(k, j, i));
              end
            end
          end
        end
