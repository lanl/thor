classdef SMat < handle
  ...
  %
  properties
    vfmin = 1.0e-08;        % smallest volume fraction, otherwise, considered zero.
  ...
  methods
    ...
    function set_mpoly_storage(obj, mesh, dim)
      % Initialize variables
      ncell_ext = mesh.ncell_prob + 2 * mesh.nbdry_prob;
      obj.nmixcell_mpoly = 0;
      % nm_sum = 0;

      if dim == 2
        % Logical indexing to find cells with multiple materials
        nm_per_cell = sum(mesh.vf_2dmat(2:ncell_ext(2)-1, 2:ncell_ext(1)-1, :) >= obj.vfmin, 3);
        mixed_cells = nm_per_cell > 1;

        % Count total mixed cells and materials
        obj.nmixcell_mpoly = sum(mixed_cells(:));
        % nm_sum = sum(nm_per_cell(mixed_cells));

      elseif dim == 3
        % Logical indexing for 3D case
        nm_per_cell = sum(mesh.vf_3dmat(2:ncell_ext(3)-1, 2:ncell_ext(2)-1, 2:ncell_ext(1)-1, :) > 0, 4);
        mixed_cells = nm_per_cell > 1;
      ...
      % Return if no mixed cells found
      if obj.nmixcell_mpoly == 0
        return;
      end

      %% compute mixcell data
      % Initialize variables
      obj.nmat_in_mixcell = zeros(obj.nmixcell_mpoly, 1);
      obj.vfgrad_mixcell = cell(obj.nmixcell_mpoly, 1);
      obj.ijk_in_mixcell = zeros(obj.nmixcell_mpoly, dim);
      obj.matids_in_mixcell = cell(obj.nmixcell_mpoly, 1);
      ...
      if dim == 2
        for j = 2:ncell_ext(2)-1
          for i = 2:ncell_ext(1)-1
            vfs = squeeze(mesh.vf_2dmat(j, i, :));
            nm = sum(vfs >= obj.vfmin);

            if nm > 1
              mx = mx + 1;
              obj.nmat_in_mixcell(mx) = nm;
              obj.ijk_in_mixcell(mx, :) = [i, j];

              matids = find(vfs >= obj.vfmin);
              obj.matids_in_mixcell{mx} = matids;

              % Allocate gradient storage dynamically
              obj.vfgrad_mixcell{mx} = zeros(nm, dim);
            end
          end
        end
      ...
      elseif dim == 3
        for k = 2:ncell_ext(3)-1
          for j = 2:ncell_ext(2)-1
            for i = 2:ncell_ext(1)-1
              vfs = squeeze(mesh.vf_3dmat(k, j, i, :));
              nm = sum(vfs >= obj.vfmin);

              if nm > 1
                mx = mx + 1;
                obj.nmat_in_mixcell(mx) = nm;
                obj.ijk_in_mixcell(mx, :) = [i, j, k];
        ...
        for i = 1:ncell_ext(1)
          if (i == 5) && (j == 4)
            [nmat_cell, matids_cell, rho, pres, ei, vel, ...
              ...
              = obj.debug_set_cell_mat(dim, xl, dx, nreg, is_solid, ...
          ...
          %for s = 1:dim
          %    mesh.vel_for_2dcell{j, i}(s) = vel(s);
          %end
    ...
    function get_mpoly(obj, mesh)
      ...
      % Set up storage
      obj.set_mpoly_storage(mesh,dim);
      % Return if no mixed cells found
      if obj.nmixcell_mpoly == 0
        return;
      end
    ...
    %%
    function [nmat_advected, matid_advected, vol_advected, mass_advected, ener_advected] = ...
        advect2d(~,nmat, ncell, nbdry, vf_2dmat, rho_2dmat, ei_2dmat, ...
        ijk, xl_cell, dx, nnode, coords, nm_this_cell, matids, ...
        nnode_for_mpoly, nodes_for_mpoly, xl_slab, xr_slab, inward_norm, plane_of_slab, vfmin)
      
      global vof2d util;
      dim = 2;
      accuracy = 1.0e-06; % Tolerance for checking on-plane points
      ...
      for n = 1:nnode
        c = coords_scaled((n-1)*dim + 1 : n*dim);
        for i = 1:dim
          c(i) = (c(i) - xl_cell(i)) / dx(i);
        end
        coords_scaled((n-1)*dim + 1 : n*dim) = c;
      end
      ic = ijk(1);
      jc = ijk(2);
      
      % Filter materials by volume fraction
      nm = length(matids);
      assert(nm == nm_this_cell);

      nmat_advected = 0; % Initialize count of advected materials
      matid_advected = [];
      vol_advected = [];
      mass_advected = [];
      ener_advected = [];
      ...
      for s = 1:nm_this_cell
        m = matids(s);
      
        nn = nnode_for_mpoly(s);
        nodes = nodes_for_mpoly{s};
      
        % Copy coordinates of material polygon
        for idx = 1:nn
          n = nodes(idx);
          coords_mpoly((idx-1)*dim + 1 : idx*dim) = coords_scaled((n-1)*dim + 1 : n*dim);
        end

        % Compute distances of nodes from the interface
        for n = 1:nn
          ds_ea_node(n) = 0.0;
          c = coords_mpoly((n-1)*dim + 1 : n*dim);
          for i = 1:dim
            ds_ea_node(n) = ds_ea_node(n) + inward_norm(i) * c(i);
          end
        end

        % Determine node locations relative to the interface
        node_loc = zeros(size(ds_ea_node));
        node_loc(abs(ds_ea_node - distance) <= accuracy) = 0; % On-plane
        node_loc(ds_ea_node > distance) = 1; % Above-plane
        node_loc(ds_ea_node < distance) = -1; % Below-plane

        % Find interface and split material polygon
        % [nnode_new, coords_new, nnode_upper, nodelist_upper] = ...
        %   find_interface2d(nn, coords_mpoly, inward_norm, distance, node_loc);
        
        [~, coords_new, ~, ~, ~, ~, nnode_upper, nodelist_upper] = ...
          vof2d.find_interface2d(nn, coords_mpoly, [1:6], inward_norm, distance, node_loc);

        % Process upper portion of the material polygon
        if nnode_upper > 2
          assert(~isempty(nodelist_upper), 'nodelist_upper cannot be empty');

          for idx = 1:nnode_upper
            n = nodelist_upper(idx);
            if n <= nn
              coords_intersect((idx-1)*dim + 1 : idx*dim) = ...
                coords_mpoly((n-1)*dim + 1 : n*dim);
            else
              coords_intersect((idx-1)*dim + 1 : idx*dim) = ...
                coords_new((n-nn-1)*dim + 1 : (n-nn)*dim);
            end
          end


          % Calculate volume, mass, and energy
          vol = util.cal_poly_area(nnode,coords_intersect,nnode_upper,[1:4]) * factor_vol;
          matid_advected = [matid_advected, m];
          vol_advected = [vol_advected, vol];
          mass_advected = [mass_advected, rho_2dmat(jc, ic, m) * vol];
          ener_advected = [ener_advected, ei_2dmat(jc, ic, m) * vol];
    ...
    function [nmat_advected, matid_advected, vol_advected, mass_advected, ener_advected] = ...
        advect3d(~, mesh, ijk, xl_cell, dx_cell, nm_this_cell, matids, xl_slab, xr_slab, ...
                 inward_norm, plane_of_slab, nnode_tot, coords_tot, nface_for_mpoly, ...
                 nnode_for_face_ea_mpoly, nodelist_for_face_ea_mpoly)
        %
        % This method calculates the advection of material properties (volume fraction,
        % density, and internal energy) across a specified slab within a 3D mesh cell.
        %
        % Inputs:
        %   obj            - (c_mat) The material object instance.
        %   mesh           - (c_mesh) The mesh object instance containing the mesh data.
        %   ijk            - (int array) The index of the current cell in the 3D mesh,
        %                    size: [3, 1].
        %   xl_cell        - (double array) The lower bounds of the cell,
        %                    size: [3, 1].
        %   dx_cell        - (double array) The dimensions of the cell,
        %                    size: [3, 1].
        %   nm_this_cell   - (int) Number of materials in this cell
        %   matids         - (int array) The material IDs in the cell,
        %                    size: [nmat_this_cell, 1].
        %   xl_slab        - (double array) The lower bounds of the slab in physical
        %                    coordinates, size: [3, 1].
        %   xr_slab        - (double array) The upper bounds of the slab in physical
        %                    coordinates, size: [3, 1].
        %   inward_norm    - (double array) Normal vector defining the inward direction,
        %                    size: [3, 1].
        %   plane_of_slab  - (int) The plane index of the slab (1:+x, 2:-x, 3:+y, 4:-y, 5:+z, 6:-z).
        %   nnode_tot      - (int) The total number of nodes in the current cell.
        %   coords_tot     - (double array) The coordinates of all nodes, size: [3*nnode_tot, 1].
        %   nface_for_mpoly - (int array) The number of faces for each material polyhedron,
        %                     size: [nmat_this_cell, 1].
        %   nnode_for_face_ea_mpoly - (cell array of int arrays) The number of nodes for
        %                             each face of each material polyhedron.
        %   nodelist_for_face_ea_mpoly - (cell array of int arrays) The list of node indices
        %                                for each face of each material polyhedron.
        %
        % Outputs:
        %   nmat_advected  - (int) The number of materials advected across the slab.
        %   matid_advected - (int array) The material IDs of the advected materials,
        %                    size: [nmat_advected, 1].
        %   vol_advected   - (double array) The volumes of the advected materials,
        %                    size: [nmat_advected, 1].
        %   mass_advected  - (double array) The masses of the advected materials,
        %                    size: [nmat_advected, 1].
        %   ener_advected  - (double array) The internal energies of the advected
        %                    materials, size: [nmat_advected, 1].
        %
        % Uses:
        %   mesh.nmat             - (int) The number of materials in the mesh.
        %   mesh.ncell            - (int array) The dimensions of the mesh in each
        %                           direction, size: [3, 1].
        %   mesh.nbdry            - (int) The number of boundary layers in the mesh.
        %   mesh.vf_3dmat         - (double 4D array) Volume fractions for each material,
        %                           size: [nmat, nx, ny, nz].
        %   mesh.rho_3dmat        - (double 4D array) Densities for each material,
        %                           size: [nmat, nx, ny, nz].
        %   mesh.ei_3dmat         - (double 4D array) Internal energies for each material,
        %                           size: [nmat, nx, ny, nz].
        %
        % Modifies:
        %   None.
        ...
        % Initialize variables
        global vof3d vof3d_lib;
        dim = 3;
        accuracy = 1.0e-06;
        
        % Scale slab coordinates
        xl_slab_scaled = (xl_slab - xl_cell) ./ dx_cell;
        xr_slab_scaled = (xr_slab - xl_cell) ./ dx_cell;
        factor_vol = prod(dx_cell);
    
        % Scale total coordinates
        coords_scaled = (coords_tot(1:nnode_tot,:) - xl_cell) ./ dx_cell;
    
        % Initialize slab plane coordinates
        %//              --------------
        %//             /.           / |
        %//            / .          /  |
        %//           /  .         /   |
        ...
        coords_plane = zeros(4, dim);
        if plane_of_slab == 1      % xl face of the slab
            coords_plane = [xl_slab_scaled(1), xl_slab_scaled(2), xl_slab_scaled(3);
                            xl_slab_scaled(1), xr_slab_scaled(2), xl_slab_scaled(3);
                            xl_slab_scaled(1), xr_slab_scaled(2), xr_slab_scaled(3);
                            xl_slab_scaled(1), xl_slab_scaled(2), xr_slab_scaled(3)];
        elseif plane_of_slab == 2  % xr face of the slab
            coords_plane = [xr_slab_scaled(1), xl_slab_scaled(2), xl_slab_scaled(3);
                            xr_slab_scaled(1), xl_slab_scaled(2), xr_slab_scaled(3);
                            xr_slab_scaled(1), xr_slab_scaled(2), xr_slab_scaled(3);
                            xr_slab_scaled(1), xr_slab_scaled(2), xl_slab_scaled(3)];
        elseif plane_of_slab == 3  % yl face of the slab
            coords_plane = [xl_slab_scaled(1), xl_slab_scaled(2), xl_slab_scaled(3);
                            xl_slab_scaled(1), xl_slab_scaled(2), xr_slab_scaled(3);
                            xr_slab_scaled(1), xl_slab_scaled(2), xr_slab_scaled(3);
                            xr_slab_scaled(1), xl_slab_scaled(2), xl_slab_scaled(3)];
        elseif plane_of_slab == 4  % yr face of the slab
            coords_plane = [xl_slab_scaled(1), xr_slab_scaled(2), xl_slab_scaled(3);
                            xr_slab_scaled(1), xr_slab_scaled(2), xl_slab_scaled(3);
                            xr_slab_scaled(1), xr_slab_scaled(2), xr_slab_scaled(3);
                            xl_slab_scaled(1), xr_slab_scaled(2), xr_slab_scaled(3)];
        elseif plane_of_slab == 5  % zl face of the slab
            coords_plane = [xl_slab_scaled(1), xl_slab_scaled(2), xl_slab_scaled(3);
                            xr_slab_scaled(1), xl_slab_scaled(2), xl_slab_scaled(3);
                            xr_slab_scaled(1), xr_slab_scaled(2), xl_slab_scaled(3);
                            xl_slab_scaled(1), xr_slab_scaled(2), xl_slab_scaled(3)];
        elseif plane_of_slab == 6  % zr face of the slab
            coords_plane = [xl_slab_scaled(1), xl_slab_scaled(2), xr_slab_scaled(3);
                            xl_slab_scaled(1), xr_slab_scaled(2), xr_slab_scaled(3);
                            xr_slab_scaled(1), xr_slab_scaled(2), xr_slab_scaled(3);
                            xr_slab_scaled(1), xl_slab_scaled(2), xr_slab_scaled(3)];
        end
    
        % Calculate d as the average distance of the plane to the origin
        inward_norm = inward_norm(:)';
        d = mean(sum(coords_plane .* inward_norm, 2));
    
        % Initialize tracking variables
        included = zeros(nnode_tot, 1);
        nodeold2new = zeros(nnode_tot, 1);
        nmat_advected = 0;
        vol_advected = [];
        matid_advected = [];
        mass_advected = [];
        ...
        % Iterate over materials in the cell
        ic = ijk(1);
        jc = ijk(2);
        kc = ijk(3);
        for idx = 1:nm_this_cell
            m = matids(idx);
    
            % Get faces and nodes for the current material
            nface = nface_for_mpoly(idx);
            nnode_for_face = nnode_for_face_ea_mpoly{idx};
            nodelist_for_face_old = nodelist_for_face_ea_mpoly{idx};
    
            % Identify included nodes
            included(:) = 0;
            nodes = nodelist_for_face_old(:);
            included(nodes) = 1;
    
            % Build new coordinate set
            coords = coords_scaled(included == 1, :);
            nodeold2new(included == 1) = 1:sum(included);
            nnode = size(coords, 1);
    
            % Map nodes in faces
            nodelist_for_face = nodeold2new(nodelist_for_face_old);

            % Split the polyhedron using the plane
            [~,~,~,~,~, nface_upper, nnode_upper, coords_upper, ...
                nnode_for_face_upper, nodelist_for_face_upper] = ...
                vof3d_lib.polyhedron_plane(nface, nnode, coords, nnode_for_face, ...
                                             nodelist_for_face, inward_norm, d);
            coords_upper = reshape(coords_upper, 3, nnode_upper)';
            
            % If the upper polyhedron exists
            if nface_upper > 3
                % Calculate the volume of the upper polyhedron
                vol = vof3d.cal_vol(nface_upper, nnode_upper, coords_upper, ...
                                    nnode_for_face_upper, nodelist_for_face_upper);
                vol = vol * factor_vol;
            
                % Record the advected volume, material ID, mass, and energy
                vol_advected(nmat_advected + 1) = vol;
                matid_advected(nmat_advected + 1) = m;
                mass_advected(nmat_advected + 1) = mesh.rho_3dmat(kc, jc, ic, m) * vol;
                ener_advected(nmat_advected + 1) = mesh.ei_3dmat(kc, jc, ic, m) * vol;
            
                % Increment the count of advected materials
                nmat_advected = nmat_advected + 1;
            end
        end
    end
