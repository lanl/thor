classdef TTBdry < handle
  ...
  methods
    ...
    %%%% TT -implementation %%%%%%
    %% Bdry cell
    function tt_bdry_cell_2d(~, ttmesh, btype_lower, btype_upper)
      % bdry_cell_2d Applies boundary conditions to 2D cells for transmitted boundaries.
      %
      % This function handles transmitted boundary conditions for the edges
      % of a 2D grid. It modifies material properties like number of materials,
      % material IDs, densities, internal energies, and pressures for boundary
      % cells.
      %
      % Inputs:
      ...
      ncell = ttmesh.ncell_prob;    % Number of cells in each spatial dimension
      nbdry = ttmesh.nbdry_prob;    % Number of boundary cells
      ncell_ext = ncell + 2 * nbdry;  % Number of cells including boundaries

      %% xl edge without corners
      var_list = {'vf_2dmat','rho_2dmat','ei_2dmat','pres_2dmat',...
                'rho_for_2dcell','ei_for_2dcell','pres_for_2dcell'};
      if btype_lower(1) == bdry_type.bdry_transmitted
        ...
        for i = 1:nbdry
          for vv = 1:numel(var_list)
          ttmesh.(var_list{vv}) = ...
            tt_set_bdry(ttmesh.(var_list{vv}),dim,[i0,i]);
          end
        end
        % end
      end
      ...
      if btype_upper(1) == bdry_type.bdry_transmitted
        ...
        for i = 1:nbdry
          ...
          for vv = 1:numel(var_list)
            ttmesh.(var_list{vv}) = ...
              tt_set_bdry(ttmesh.(var_list{vv}),dim,[i0,i1]);
          end
        end
        % end
      end
      ...
      if btype_lower(2) == bdry_type.bdry_transmitted
        ...
        for j = 1:nbdry
          for vv = 1:numel(var_list)
            ttmesh.(var_list{vv}) = ...
            tt_set_bdry(ttmesh.(var_list{vv}),dim,[j0,j]);
          end
        end
      end
      ...
      if btype_upper(2) == bdry_type.bdry_transmitted
        ...
        for j = 1:nbdry
          ...
          for vv = 1:numel(var_list)
            ttmesh.(var_list{vv}) = ...
              tt_set_bdry(ttmesh.(var_list{vv}),dim,[j0,j1]);
          end
        end
      end
    end % tt_bdry_cell_2d
    
    function tt_bdry_cell_3d(~, ttmesh, btype_lower, btype_upper)
      % bdry_cell_2d Applies boundary conditions to 2D cells for transmitted boundaries.
      %
      % This function handles transmitted boundary conditions for the edges
      % of a 2D grid. It modifies material properties like number of materials,
      ...
      %% xl edge without corners
      var_list = {'vf_3dmat','rho_3dmat','ei_3dmat','pres_3dmat',...
                'rho_for_3dcell','ei_for_3dcell','pres_for_3dcell'};
      if btype_lower(1) == bdry_type.bdry_transmitted
        dim = 3;
        i0 = nbdry + 1;
        % for j = nbdry + 1:ncell(2) + nbdry
      ...
      if btype_lower(2) == bdry_type.bdry_transmitted
        ...
        for k = 1:nbdry
          for vv = 1:numel(var_list)
            ttmesh.(var_list{vv}) = ...
            tt_set_bdry(ttmesh.(var_list{vv}),dim,[k0,k]);
          end
        end
      end

      %% zr edge including corners
      if btype_upper(2) == bdry_type.bdry_transmitted
        dim = 1;
        k0 = ncell(3) + nbdry;
        for k = 1:nbdry
          ...
          for vv = 1:numel(var_list)
            ttmesh.(var_list{vv}) = ...
              tt_set_bdry(ttmesh.(var_list{vv}),dim,[k0,k1]);
          end
        end
      end
    end % tt_bdry_cell_3d
    %% Bdry node
    function tt_bdry_node_2d(~, ttmesh, btype_lower, btype_upper)
      ncell = ttmesh.ncell_prob;   % Number of cells in each direction
      nbdry = ttmesh.nbdry_prob;   % Number of boundary cells
      nnode = ncell + 1;
      ...
      if btype_lower(1) == bdry_type.bdry_transmitted
        ...
        %tt
        for i = 1:nbdry
          ttmesh.vel_for_2dnode = ...
            tt_set_bdry(ttmesh.vel_for_2dnode, tempdim, [n0,i]);
          ttmesh.vav_for_2dnode = ...
            tt_set_bdry(ttmesh.vav_for_2dnode, tempdim, [n0,i]);
        end
      end
      ...
      if btype_upper(1) == bdry_type.bdry_transmitted
        ...
        for i = 1:nbdry
          n1 = n0 + i;
          ttmesh.vel_for_2dnode = ...
            tt_set_bdry(ttmesh.vel_for_2dnode, tempdim, [n0,n1]);
          ttmesh.vav_for_2dnode = ...
            tt_set_bdry(ttmesh.vav_for_2dnode, tempdim, [n0,n1]);
        end
      end
      ...
      if btype_lower(2) == bdry_type.bdry_transmitted
        ...
        for j = 1:nbdry
          ttmesh.vel_for_2dnode = ...
            tt_set_bdry(ttmesh.vel_for_2dnode, tempdim, [n0,j]);
          ttmesh.vav_for_2dnode = ...
            tt_set_bdry(ttmesh.vav_for_2dnode, tempdim, [n0,j]);
        end
      end
    ...
    %%%%%%%%
    function tt_bdry_node_3d(~, ttmesh, btype_lower, btype_upper)
      ncell = ttmesh.ncell_prob;   % Number of cells in each direction
      nbdry = ttmesh.nbdry_prob;   % Number of boundary cells
      nnode = ncell + 1;
      ...
      if btype_lower(1) == bdry_type.bdry_transmitted
        ...
        %tt
        for i = 1:nbdry
          ttmesh.vel_for_3dnode = ...
            tt_set_bdry(ttmesh.vel_for_3dnode, tempdim, [n0,i]);
          ttmesh.vav_for_3dnode = ...
            tt_set_bdry(ttmesh.vav_for_3dnode, tempdim, [n0,i]);
        end
      end
      ...
      if btype_upper(1) == bdry_type.bdry_transmitted
        ...
        for i = 1:nbdry
          n1 = n0 + i;
          ttmesh.vel_for_3dnode = ...
            tt_set_bdry(ttmesh.vel_for_3dnode, tempdim, [n0,n1]);
          ttmesh.vav_for_3dnode = ...
            tt_set_bdry(ttmesh.vav_for_3dnode, tempdim, [n0,n1]);
        end
      end
      ...
      if btype_lower(2) == bdry_type.bdry_transmitted
        ...
        for j = 1:nbdry
          ttmesh.vel_for_3dnode = ...
            tt_set_bdry(ttmesh.vel_for_3dnode, tempdim, [n0,j]);
          ttmesh.vav_for_3dnode = ...
            tt_set_bdry(ttmesh.vav_for_3dnode, tempdim, [n0,j]);
        end
      end
      ...
      if btype_lower(3) == bdry_type.bdry_transmitted
        ...
        for k = 1:nbdry
          ttmesh.vel_for_3dnode = ...
            tt_set_bdry(ttmesh.vel_for_3dnode, tempdim, [n0,k]);
          ttmesh.vav_for_3dnode = ...
            tt_set_bdry(ttmesh.vav_for_3dnode, tempdim, [n0,k]);
        end
      end
    ...
    function cell_bdry_condition(obj, ttmesh, btype_lower, btype_upper)
      % cell_bdry_condition: simple switch

      if ttmesh.dim_prob == 2

        %tt
        obj.tt_bdry_cell_2d(ttmesh, btype_lower, btype_upper);
        obj.tt_bdry_node_2d(ttmesh, btype_lower, btype_upper);
        
      elseif ttmesh.dim_prob == 3
     
        obj.tt_bdry_cell_3d(ttmesh, btype_lower, btype_upper);
        obj.tt_bdry_node_3d(ttmesh, btype_lower, btype_upper);
      end
    end
    ...
    function bdry_cell_3d(~, mesh, btype_lower, btype_upper)
      ...
      if btype_lower(1) == bdry_type.bdry_transmitted
        ...
        for k = nbdry + 1:ncell(3) + nbdry
          for j = nbdry + 1:ncell(2) + nbdry
            for i = 1:nbdry
              mesh.nmat_for_3dcell(k, j, i) = mesh.nmat_for_3dcell(k, j, i0);
              mesh.matid_for_3dcell(k, j, i) = mesh.matid_for_3dcell(k, j, i0);
              mesh.rho_for_3dcell(k, j, i) = mesh.rho_for_3dcell(k, j, i0);
              mesh.ei_for_3dcell(k, j, i) = mesh.ei_for_3dcell(k, j, i0);
              mesh.pres_for_3dcell(k, j, i) = mesh.pres_for_3dcell(k, j, i0);
            end
          end
        end
      end
      ...
      if btype_upper(1) == bdry_type.bdry_transmitted
        ...
        for k = nbdry + 1:ncell(3) + nbdry
          for j = nbdry + 1:ncell(2) + nbdry
            for i = 1:nbdry
              i1 = i0 + i;
              mesh.nmat_for_3dcell(k, j, i1) = mesh.nmat_for_3dcell(k, j, i0);
              mesh.matid_for_3dcell(k, j, i1) = mesh.matid_for_3dcell(k, j, i0);
              mesh.rho_for_3dcell(k, j, i1) = mesh.rho_for_3dcell(k, j, i0);
              mesh.ei_for_3dcell(k, j, i1) = mesh.ei_for_3dcell(k, j, i0);
              mesh.pres_for_3dcell(k, j, i1) = mesh.pres_for_3dcell(k, j, i0);
            end
          end
        end
      end
      ...
      if btype_lower(2) == bdry_type.bdry_transmitted
        ...
        for k = nbdry + 1:ncell(3) + nbdry
          for j = 1:nbdry
            for i = 1:ncell_ext(1)
              mesh.nmat_for_3dcell(k, j, i) = mesh.nmat_for_3dcell(k, j0, i);
              mesh.matid_for_3dcell(k, j, i) = mesh.matid_for_3dcell(k, j0, i);
              mesh.rho_for_3dcell(k, j, i) = mesh.rho_for_3dcell(k, j0, i);
              mesh.ei_for_3dcell(k, j, i) = mesh.ei_for_3dcell(k, j0, i);
              mesh.pres_for_3dcell(k, j, i) = mesh.pres_for_3dcell(k, j0, i);
            end
          end
        end
      end
      ...
      if btype_upper(2) == bdry_type.bdry_transmitted
        ...
        for k = nbdry + 1:ncell(3) + nbdry
          for j = 1:nbdry
            j1 = j0 + j;
            for i = 1:ncell_ext(1)
              mesh.nmat_for_3dcell(k, j1, i) = mesh.nmat_for_3dcell(k, j0, i);
              mesh.matid_for_3dcell(k, j1, i) = mesh.matid_for_3dcell(k, j0, i);
              mesh.rho_for_3dcell(k, j1, i) = mesh.rho_for_3dcell(k, j0, i);
              mesh.ei_for_3dcell(k, j1, i) = mesh.ei_for_3dcell(k, j0, i);
              mesh.pres_for_3dcell(k, j1, i) = mesh.pres_for_3dcell(k, j0, i);
            end
          end
        end
      end
      ...
      if btype_lower(3) == bdry_type.bdry_transmitted
        ...
        for k = 1:nbdry
          for j = 1:ncell_ext(2)
            for i = 1:ncell_ext(1)
              mesh.nmat_for_3dcell(k, j, i) = mesh.nmat_for_3dcell(k0, j, i);
              mesh.matid_for_3dcell(k, j, i) = mesh.matid_for_3dcell(k0, j, i);
              mesh.rho_for_3dcell(k, j, i) = mesh.rho_for_3dcell(k0, j, i);
              mesh.ei_for_3dcell(k, j, i) = mesh.ei_for_3dcell(k0, j, i);
              mesh.pres_for_3dcell(k, j, i) = mesh.pres_for_3dcell(k0, j, i);
            end
          end
        end
      end
      ...
      if btype_upper(3) == bdry_type.bdry_transmitted
        ...
        for k = 1:nbdry
          ...
          for j = 1:ncell_ext(2)
            for i = 1:ncell_ext(1)
              mesh.nmat_for_3dcell(k1, j, i) = mesh.nmat_for_3dcell(k0, j, i);
              mesh.matid_for_3dcell(k1, j, i) = mesh.matid_for_3dcell(k0, j, i);
              mesh.rho_for_3dcell(k1, j, i) = mesh.rho_for_3dcell(k0, j, i);
              mesh.ei_for_3dcell(k1, j, i) = mesh.ei_for_3dcell(k0, j, i);
              mesh.pres_for_3dcell(k1, j, i) = mesh.pres_for_3dcell(k0, j, i);
            end
          end
        end
      end
      ...
      if btype_lower(1) == bdry_type.bdry_transmitted
        ...
        for j = nbdry+1:ncell(2) + nbdry
          for i = 1:nbdry
            ...
            for idx = 1:nm
              obj.matid_for_cell{j, i}(idx) = obj.matid_for_cell{j, i0}(idx);
              obj.vol_for_cell{j, i}(idx) = obj.vol_for_cell{j, i0}(idx);
              obj.mass_for_cell{j, i}(idx) = obj.mass_for_cell{j, i0}(idx);
              obj.ener_for_cell{j, i}(idx) = obj.ener_for_cell{j, i0}(idx);
            end
          end
        end
      end
      ...
      if btype_upper(1) == bdry_type.bdry_transmitted
        ...
        for j = nbdry+1:ncell(2) + nbdry
          for i = 1:nbdry
            ...
            nm = obj.nmat_for_cell(j, i0);
            for idx = 1:nm
              obj.matid_for_cell{j, i1}(idx) = obj.matid_for_cell{j, i0}(idx);
              obj.vol_for_cell{j, i1}(idx) = obj.vol_for_cell{j, i0}(idx);
              obj.mass_for_cell{j, i1}(idx) = obj.mass_for_cell{j, i0}(idx);
              obj.ener_for_cell{j, i1}(idx) = obj.ener_for_cell{j, i0}(idx);
            end
          end
        end
      end
      ...
      if btype_lower(2) == bdry_type.bdry_transmitted
        ...
        for j = 1:nbdry
          for i = 1:ncell_ext(1)
            ...
            for idx = 1:nm
              obj.matid_for_cell{j, i}(idx) = obj.matid_for_cell{j0, i}(idx);
              obj.vol_for_cell{j, i}(idx) = obj.vol_for_cell{j0, i}(idx);
              obj.mass_for_cell{j, i}(idx) = obj.mass_for_cell{j0, i}(idx);
              obj.ener_for_cell{j, i}(idx) = obj.ener_for_cell{j0, i}(idx);
            end
          end
        end
      end
      ...
      if btype_upper(2) == bdry_type.bdry_transmitted
        ...
        for j = 1:nbdry
          ...
          for i = 1:ncell_ext(1)
            ...
            nm = obj.nmat_for_cell(j0, i);
            for idx = 1:nm
              obj.matid_for_cell{j1, i}(idx) = obj.matid_for_cell{j0, i}(idx);
              obj.vol_for_cell{j1, i}(idx) = obj.vol_for_cell{j0, i}(idx);
              obj.mass_for_cell{j1, i}(idx) = obj.mass_for_cell{j0, i}(idx);
              obj.ener_for_cell{j1, i}(idx) = obj.ener_for_cell{j0, i}(idx);
            end
          end
        end
      end
