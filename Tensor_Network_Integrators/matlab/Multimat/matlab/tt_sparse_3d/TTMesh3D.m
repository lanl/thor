classdef TTMesh3D < handle
  ...
  properties
    ...
    nmat_for_3dcell  = [];   % the number of materials in each 2D cell
  ...
  methods
    ...
    function courant_cs = tt_courant_from_cs(obj, dt)
      ...
      if dim == 3
        ...
        courant_cs = tt_max(obj.cs_for_3dcell);
      end
    ...
    function tt_mesh_sound_speed(obj, nmat, matids, is_solid, gamma_ea_mat)
      
      global eos tiny tt_tol;  % Global object for EOS (Equation of State)
      ...
      if dim == 3
        tempfun = @(k,j,i) fun_3D(obj,k,j,i,gamma_ea_mat,tiny,is_solid,nmat);
        %
        obj.cs_for_3dcell = amen_cross_zero([ncell_ext(3),ncell_ext(2),ncell_ext(1)],...
    ...
    end %mesh_sound_speed
    %% subroutines
    %2D
    function cs_for_2dcell = fun_2D(obj,j,i,gamma_ea_mat,tiny,is_solid,nmat)
      cs_for_2dcell = 0;
      for m = 1: nmat
        if obj.vf_3dmat(j,i,m)>0
          if ~is_solid(m)
          ...
          cs_for_2dcell = cs_for_2dcell + obj.vf_2dmat(j,i,m)*cs;
        end
      end
    
    end
    %3D
    function cs_for_3dcell = fun_3D(obj,k,j,i,gamma_ea_mat,tiny,is_solid,nmat)
      cs_for_3dcell = 0;
      for m = 1: nmat
        if obj.vf_3dmat(k,j,i,m)>1e-12
          if ~is_solid(m)
            cs = sqrt(gamma_ea_mat(m) * obj.pres_3dmat(k,j,i,m) / ...
              (obj.rho_3dmat(k,j,i,m) + tiny));
          else
          ...
          cs_for_3dcell = cs_for_3dcell + obj.vf_3dmat(k,j,i,m)*cs;
        end
      end
