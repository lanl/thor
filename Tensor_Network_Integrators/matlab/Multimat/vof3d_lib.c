...
void testf(double x, double *xp, double **xpp, int n, int *np, int **npp) {
    ...
    xp[0] = 2.3;
    double **ypp;
    ypp = (double**)malloc(sizeof(double*)*2);
    ypp[0] = xpp[0];
    ypp[1] = ypp[0] + 2;
    //printf("%5.2f %5.2f %5.2f %5.2f\n", xpp[0][0], xpp[0][1], xpp[1][0], xpp[1][1]);
    //ypp[0] = xpp[0];
    //ypp[1] = xpp[0] + 2;
    printf("%5.2f %5.2f %5.2f %5.2f\n", ypp[0][0], ypp[0][1], ypp[0][2], ypp[0][3]);
    ypp[0][0] = -2.0;
    ypp[0][1] = -2.1;
    ypp[1][0] = -5.0;
    ypp[1][1] = -5.1;
    //np = (int*) malloc(10*sizeof(int));
    for (i=0;i<10;i++) np[i] = -i;
    free(ypp);
    return;
}
...
void matlab_interface3d(double dx_scale,
    ...
    if (verbose) {
        ...
        printf ("nnode_for_face = {");
        for (i = 0; i < nface; i++) printf("%4d", nnode_for_face[i]);
        printf("}\n");

        printf ("nodelist_for_face = {\n");
        offset = 0;
        for (i = 0; i < nface; i++) {
            printf("    ");
            for (j = 0; j < nnode_for_face[i]; j++) {
                printf("%4d, ", nodelist_for_face[offset]);
                offset++;
            }
            printf("\n");
        }
        ...
        printf("cell_vol = %f\n", cell_vol);
        printf("vf_to_match = %f\n", vf_to_match);
        printf("normal = {%14.7f %14.7f %14.7f}\n", normal[0], normal[1], normal[2]);
    }

    int ifinterface, nnode_new;
    double *coords_new;
    int nnode_for_interface, *nodelist_for_interface,
        faceindexlow_of_interface, faceindexhgh_of_interface,
        *ifintersected_face, *nodelist_intersected_face, *nodes2_in_between,
        nface_lower, *nnode_for_face_lower, *nodelist_for_face_lower, *face2old_lower,
        nface_upper, *nnode_for_face_upper, *nodelist_for_face_upper, *face2old_upper;
    ...
    nodelist_intersected_face = (int *) malloc(2*nface * sizeof(int));
    nodes2_in_between         = NULL;
    ...
    *out_nnode_new = nnode_new;
    for (i = 0; i < *out_nnode_new; i++) {
        out_coords_new[3*i] = coords_new[3*i];
        out_coords_new[3*i+1] = coords_new[3*i+1];
        out_coords_new[3*i+2] = coords_new[3*i+2];
    }

    *out_nnode_for_interface = nnode_for_interface;
    for (i = 0; i < (*out_nnode_for_interface); i++) {
        out_nodelist_for_interface[i] = nodelist_for_interface[i];
    }

    *out_faceindexlow_of_interface = faceindexlow_of_interface;
    *out_faceindexhgh_of_interface = faceindexhgh_of_interface;

    *out_nface_lower = nface_lower;
    for (i = 0; i < *out_nface_lower; i++) {
        out_nnode_for_face_lower[i] = nnode_for_face_lower[i];
    }
    offset = 0;
    for (i = 0; i < *out_nface_lower; i++) {
        for(j = 0; j < out_nnode_for_face_lower[i]; j++) {
            out_nodelist_for_face_lower[offset] = nodelist_for_face_lower[offset];
            offset++;
        }
    }

    *out_nface_upper = nface_upper;
    for (i = 0; i < *out_nface_upper; i++) {
        out_nnode_for_face_upper[i] = nnode_for_face_upper[i];
    }
    offset = 0;
    for (i = 0; i < *out_nface_upper; i++) {
        for(j = 0; j < out_nnode_for_face_upper[i]; j++) {
            out_nodelist_for_face_upper[offset] = nodelist_for_face_upper[offset];
            offset++;
        }
    }

    free(coords_new);
    free(nodelist_for_interface);
    ...
    if (verbose) {
        ...
        for (i = 0; i < *out_nnode_new; i++) {
            printf("%14.7f %14.7f %14.7f,\n",
                out_coords_new[3*i],
                out_coords_new[3*i+1],
                out_coords_new[3*i+2]);
        }
        printf ("}\n");

        printf("nnode_for_interface = %d\n", *out_nnode_for_interface);
        printf("nodelist_for_interface = {\n");
        for (i = 0; i < (*out_nnode_for_interface); i++) {
            printf("%4d, ", out_nodelist_for_interface[i]);
        }
        printf("}\n");

        printf("faceindexlow_of_interface = %d\n", *out_faceindexlow_of_interface);
        printf("faceindexhgh_of_interface = %d\n", *out_faceindexhgh_of_interface);

        printf("nface_lower = %d\n", *out_nface_lower);
        printf("nnode_for_face_lower = {");
        for (i = 0; i < *out_nface_lower; i++) {
            printf("%d, ", out_nnode_for_face_lower[i]);
        }
        printf("}\n");
        offset = 0;
        printf("nodelist_for_face_lower = {");
        for (i = 0; i < *out_nface_lower; i++) {
            for(j = 0; j < out_nnode_for_face_lower[i]; j++) {
                printf("%d, ", out_nodelist_for_face_lower[offset]);
                offset++;
            }
            printf("\n");
        }
        ...
        *out_nface_upper = nface_upper;
        printf("nface_upper = %d\n", *out_nface_upper);
        printf("nnode_for_face_upper = {");
        for (i = 0; i < *out_nface_upper; i++) {
            printf("%d, ", out_nnode_for_face_upper[i]);
        }
        printf("}\n");
        offset = 0;
        printf("nodelist_for_face_upper = {");
        for (i = 0; i < *out_nface_upper; i++) {
            for(j = 0; j < out_nnode_for_face_upper[i]; j++) {
                printf("%d, ", out_nodelist_for_face_upper[offset]);
                offset++;
            }
            printf("\n");
        }
...
void matlab_polyhedron_plane(int nface, int nnode, double *coords,
    ...
    if (verbose) {
        ...
        printf ("nnode_ea_face = {");
        for (i = 0; i < nface; i++) printf("%4d", nnode_ea_face[i]);
        printf("}\n");
        ...
        printf("norm_plane = {%14.7f %14.7f %14.7f}\n", 
            norm_plane[0], norm_plane[1], norm_plane[2]);
        printf ("ds_plane = %f\n", ds_plane);
    }

    double *coords_lower, *coords_upper;
    int nface_lower, nnode_lower, *nnode_ea_face_lower, *nodelist_for_face_lower,
        nface_upper, nnode_upper, *nnode_ea_face_upper, *nodelist_for_face_upper;
    ...
    *out_nface_lower = nface_lower;
    *out_nnode_lower = nnode_lower;
    for (i = 0; i < *out_nnode_lower; i++) {
        out_coords_lower[3*i]   = coords_lower[3*i];
        out_coords_lower[3*i+1] = coords_lower[3*i+1];
        out_coords_lower[3*i+2] = coords_lower[3*i+2];
    }
    offset = 0;
    for (i = 0; i < *out_nface_lower; i++) {
        out_nnode_ea_face_lower[i] = nnode_ea_face_lower[i];
        for (j = 0; j < out_nnode_ea_face_lower[i]; j++) {
            out_nodelist_for_face_lower[offset] = nodelist_for_face_lower[offset];
            offset++;
        }
    }
    ...
    *out_nface_upper = nface_upper;
    *out_nnode_upper = nnode_upper;
    for (i = 0; i < *out_nnode_upper; i++) {
        out_coords_upper[3*i]   = coords_upper[3*i];
        out_coords_upper[3*i+1] = coords_upper[3*i+1];
        out_coords_upper[3*i+2] = coords_upper[3*i+2];
    }
    offset = 0;
    for (i = 0; i < *out_nface_upper; i++) {
        out_nnode_ea_face_upper[i] = nnode_ea_face_upper[i];
        for (j = 0; j < out_nnode_ea_face_upper[i]; j++) {
            out_nodelist_for_face_upper[offset] = nodelist_for_face_upper[offset];
            offset++;
        }
    }
    ...
    if (nnode_ea_face_lower)     free(nnode_ea_face_lower);
    if (nodelist_for_face_lower) free(nodelist_for_face_lower);

    if (coords_upper)            free(coords_upper);
    if (nnode_ea_face_upper)    free(nnode_ea_face_upper);
    ...
    if (verbose) {
        ...
        printf("---- OUTPUT ----\n");
        printf ("nface_lower  = %d\n", *out_nface_lower);
        printf ("nnode_lower  = %d\n", *out_nnode_lower);

        printf ("coords_lower  = {\n");
        for (i = 0; i < *out_nnode_lower; i++) {
            printf("%14.7f %14.7f %14.7f,\n",
                out_coords_lower[3*i],
                out_coords_lower[3*i+1],
                out_coords_lower[3*i+2]);
        }
        printf ("}\n");

        printf ("nface_lower = %d\n", *out_nface_lower);
        printf ("nnode_ea_face_lower = {");
        for (i = 0; i < *out_nface_lower; i++) {
            printf("%4d ", out_nnode_ea_face_lower[i]);
        }
        printf("}\n");
        offset = 0;
        printf ("nodelist_for_face_lower = {\n");
        for (i = 0; i < *out_nface_lower; i++) {
            for (j = 0; j < out_nnode_ea_face_lower[i]; j++) {
                printf("%4d ", out_nodelist_for_face_lower[offset]);
                offset++;
            }
            printf("\n");
        }
        ...
        printf ("nface_upper  = %d\n", *out_nface_upper);
        printf ("nnode_upper  = %d\n", *out_nnode_upper);

        printf ("coords_upper  = {\n");
        for (i = 0; i < *out_nnode_upper; i++) {
            printf("%14.7f %14.7f %14.7f,\n",
                out_coords_upper[3*i],
                out_coords_upper[3*i+1],
                out_coords_upper[3*i+2]);
        }
        printf ("}\n");

        printf ("nface_upper = %d\n", *out_nface_upper);
        printf ("nnode_ea_face_upper = {");
        for (i = 0; i < *out_nface_upper; i++) {
            printf("%4d ", out_nnode_ea_face_upper[i]);
        }
        printf("}\n");
        offset = 0;
        printf ("nodelist_for_face_upper = {\n");
        for (i = 0; i < *out_nface_upper; i++) {
            for (j = 0; j < out_nnode_ea_face_upper[i]; j++) {
                printf("%4d ", out_nodelist_for_face_upper[offset]);
                offset++;
            }
            printf("\n");
        }
