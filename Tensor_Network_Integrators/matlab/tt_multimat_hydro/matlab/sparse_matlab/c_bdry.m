        ...
        function bdry_cell_2d(~, mesh, btype_lower, btype_upper)
            ...
            if btype_lower(1) == bdry_type.bdry_transmitted
                ...
                for j = nbdry + 1:ncell(2) + nbdry
                    for i = 1:nbdry
                        mesh.nmat_for_2dcell(j, i) = mesh.nmat_for_2dcell(j, i0);
                        mesh.matid_for_2dcell(j, i) = mesh.matid_for_2dcell(j, i0);
                        mesh.rho_for_2dcell(j, i) = mesh.rho_for_2dcell(j, i0);
                        mesh.ei_for_2dcell(j, i) = mesh.ei_for_2dcell(j, i0);
                        mesh.pres_for_2dcell(j, i) = mesh.pres_for_2dcell(j, i0);
                    end
                end
            end
            ...
            if btype_upper(1) == bdry_type.bdry_transmitted
                ...
                for j = nbdry + 1:ncell(2) + nbdry
                    for i = 1:nbdry
                        i1 = i0 + i;
                        mesh.nmat_for_2dcell(j, i1) = mesh.nmat_for_2dcell(j, i0);
                        mesh.matid_for_2dcell(j, i1) = mesh.matid_for_2dcell(j, i0);
                        mesh.rho_for_2dcell(j, i1) = mesh.rho_for_2dcell(j, i0);
                        mesh.ei_for_2dcell(j, i1) = mesh.ei_for_2dcell(j, i0);
                        mesh.pres_for_2dcell(j, i1) = mesh.pres_for_2dcell(j, i0);
                    end
                end
            end
            ...
            if btype_lower(2) == bdry_type.bdry_transmitted
                ...
                for j = 1:nbdry
                    for i = 1:ncell_ext(1)
                        mesh.nmat_for_2dcell(j, i) = mesh.nmat_for_2dcell(j0, i);
                        mesh.matid_for_2dcell(j, i) = mesh.matid_for_2dcell(j0, i);
                        mesh.rho_for_2dcell(j, i) = mesh.rho_for_2dcell(j0, i);
                        mesh.ei_for_2dcell(j, i) = mesh.ei_for_2dcell(j0, i);
                        mesh.pres_for_2dcell(j, i) = mesh.pres_for_2dcell(j0, i);
                    end
                end
            end
            ...
            if btype_upper(2) == bdry_type.bdry_transmitted
                ...
                for j = 1:nbdry
                    j1 = j0 + j;
                    for i = 1:ncell_ext(1)
                        mesh.nmat_for_2dcell(j1, i) = mesh.nmat_for_2dcell(j0, i);
                        mesh.matid_for_2dcell(j1, i) = mesh.matid_for_2dcell(j0, i);
                        mesh.rho_for_2dcell(j1, i) = mesh.rho_for_2dcell(j0, i);
                        mesh.ei_for_2dcell(j1, i) = mesh.ei_for_2dcell(j0, i);
                        mesh.pres_for_2dcell(j1, i) = mesh.pres_for_2dcell(j0, i);
                    end
                end
            end
        end
...
%%%%%%%%%%%%%%% implemented but not tested
        ...
        function bdry_node_3d(~, mesh, btype_lower, btype_upper)
            ...
            dim = 3;
            nnode = mesh.ncell_prob + 1;   % Number of cells in each direction
            nbdry = mesh.nbdry_prob;   % Number of boundary cells
            nnode_ext = nnode + 2 * nbdry;  % Extended cell size (including boundaries)
