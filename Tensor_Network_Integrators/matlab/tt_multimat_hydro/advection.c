...
void advection(int fileid, int dim, int *ncell, int nbdry, double *xl_prob, 		
				double *dx_prob,
				int nmat,  int *solid_num_ea_mat, double *gamma_ea_mat,
				int *if_fixed_state_ea_mat, 
				double *rho_fixed_state_ea_mat, double *ei_fixed_state_ea_mat, double *pres_fixed_state_ea_mat,
...
//   get face velocity of the cell interface in the direction of dir.
	 ...
	 // #ifdef H5DUMP_DEBUG
	 //     // printf('debug in advection is activated \n');
	 //     if (dim == 2)
	 //         if (dir == 0)
	 //         {
	 //             ntemp[0] = nnode_ext[0];
				 ...
				 double *courant_adv)
{
	 ...
	 double tmp, rho_sum, vol_sum, mass_sum, ener_sum;
	 double *gamma_matid;

	 double **mass_for_node; 
	 ...
	 int if_mom_homoginized = 1;
	 int if_mom_conserved   = 1;
				 ...
				 *courant_adv = MAX(*courant_adv, 2.0 * fabs(frac));
				 ...
				 if (frac > small) {  // cell clower moves the following mass and energy to cupper
					 ...
					 if (nm < 1) {
						 printf("j = %d, i = %d\n", cupper, i);
						 for (m = 0; m < nmat; m++) { 
							 printf("vf_2dmat[m=%d] = %e\n", m, vf_2dmat[cupper][i][m]);
						 }
					 }
					 assert(nm > 0);
					 adist = fabs(dist);
...
#pragma region mom_for_node
	 ...
	 if (if_mom_conserved) { 
		 ...
		 mass_for_node = (double **) malloc(nnode_ext[1] * sizeof(double *));
		 mass_for_node[0] = (double *) malloc(nnode_ext[1] * nnode_ext[0] * sizeof(double));
		 for (j = 1; j < nnode_ext[1]; j++) { 
			 mass_for_node[j] = mass_for_node[j-1] + nnode_ext[0];
		 } 
		 for (j = 1; j < nnode_ext[1]-1; j++) {
			 for (i = 1; i < nnode_ext[0]-1; i++) {
				 mass_for_node[j][i] = tiny; 
				 for (jc = j-1; jc <= j; jc++) {
					 for (ic = i-1; ic <= i; ic++) {
						 nm = 0;
						 for (m = 0; m < nmat; m++) {
							 if (vf_2dmat[jc][ic][m] >= vfmin) {
								 mats[nm] = m;
								 nm++;
							 }
						 }
						 assert(nm > 0);
						 if ((nm == 1)||if_mom_homoginized) {
							 mass_for_node[j][i] += (0.25 * vol_cell * rho_2dcell[jc][ic]);
						 } 
						 else { 
							 ijk[0] = ic;
							 ijk[1] = jc; 
							 ...
							 coord_node[0] = xl_prob[0] + (double)(i - nbdry) * dx[0];
							 coord_node[1] = xl_prob[1] + (double)(j - nbdry) * dx[1];

							 xl_qtr[0]  = coord_node[0] + (double)(ic - i) * hdx[0];
							 xl_qtr[1]  = coord_node[1] + (double)(jc - j) * hdx[1];

							 intersect2d(nmat, ncell, nbdry, 
										 rho_2dmat, ei_2dmat, 
										 ijk, nm, mats, 
										 xl_qtr, hdx, 
							 ...
							 for (idx = 0; idx < nmat_adv; idx++) {
								 vol_sum += vol_adv[idx]; 
								 mass_for_node[j][i] += mass_adv[idx];
							 }
							 tmp = (vol_sum/vol_qtr - 1.0);
							 assert(fabs(tmp) < 1.0e-05); 
						 } 
					 }
				 }
				 for (idx = 0; idx < dim; idx++) {
					 mom_for_node[j][i][idx] =  mass_for_node[j][i] * vel_2dnode[j][i][idx];
				 }
			 }
		 }
	 }
	 else { 
		 memcpy(mom_for_node[0][0], vel_2dnode[0][0], (size_t)(lsize_node * dim * sizeof(double)));
	 } 
	 ...
	 for (j = nbdry; j <= ncell_bdry[1]; j++) {
		 for (i = nbdry; i <= ncell_bdry[0]; i++) {
			 for (m = 0; m < nmat; m++) { 
				 ...
				 if (if_fixed_state_ea_mat[m]) { 
//DAI
					 rho_2dmat[j][i][m]  = rho_fixed_state_ea_mat[m];
//                   rho_2dmat[j][i][m] = mass_for_cell[j][i][m]/(tiny + vol_for_cell[j][i][m]);

					 ei_2dmat[j][i][m]   = ei_fixed_state_ea_mat[m];
					 pres_2dmat[j][i][m] = pres_fixed_state_ea_mat[m];
				 }
				 else {  
					 rho_2dmat[j][i][m] = mass_for_cell[j][i][m]/(tiny + vol_for_cell[j][i][m]);
					 ...
					 if (!solid_num_ea_mat[m]) { 
						 ...
						 p_solid(solid_num_ea_mat[m], rho_2dmat[j][i][m], ei_2dmat[j][i][m], &(pres_2dmat[j][i][m]));
					 }               
				 } 
				 ei_2dmat[j][i][m] = MAX(ei_2dmat[j][i][m], 0.0);
				 pres_2dmat[j][i][m] = MAX(pres_2dmat[j][i][m], 0.0);
			 }
		 }
	 }   
	 for (j = nbdry; j <= ncell_bdry[1]; j++) {
			 ...
			 for (i = nbdry; i <= nnode_bdry[0]; i++) {
				 ...
				 for (jc = j-1; jc <= j; jc++) {
					 for (ic = i-1; ic <= i; ic++) {
						 ...
						 assert(nm > 0);
						 if ((nm == 1)||if_mom_homoginized) {
							 dmass += (vol_qtr * rho_2dcell[jc][ic]);
						 }
						 else {  
							 ijk[0] = ic;
							 ijk[1] = jc;
...
//DAI 
//                 dmass = MAX(dmass, mass_for_node[j][i]);

				 for (idx = 0; idx < dim; idx++) {
					 vel_2dnode[j][i][idx] = mom_for_2dnode_new[j][i][idx]/dmass;
//                   vel_2dnode[j][i][idx] = mom_for_2dnode_new[j][i][idx]/mass_for_node[j][i]; 
				 }
			 }
		 }
	 }
	 else {
		 memcpy(vel_2dnode[0][0], mom_for_2dnode_new[0][0], (size_t)(lsize_node * dim * sizeof(double)));
	 }
#pragma endregion
...
void mapping3d(int *ncell, int nbdry, double *xl_prob, double *dx, int dir, double dt,
			 ...
			 double *rho_fixed_state_ea_mat, double *ei_fixed_state_ea_mat, double *pres_fixed_state_ea_mat,
				 ...
				 double *courant_adv)
{
	int dim, i, j, k, ip, jp, kp, ic, jc, kc, ic_in, ic_out, jc_in, jc_out, kc_in, kc_out, idx, idxe, edge;
	int clower, cupper, m, matid, nmat_adv;
	...
	double vol_cell, vol;
	double frac, x_edge, y_edge, z_edge, dist, adist, dvol, dmass, dener, dmom;
	double inward_norm[3], xl_cell[3], xl_slab[3], xr_slab[3];
	double hdx[3], coord_node[3], xl_qtr[3];
	double *dvol_for_edge, *dmass_for_edge, *dener_for_edge;
...
//   -----------------
			...
			for (j = 1; j < nnode_ext[1]-1; j++) {
				for (i = 1; i < nnode_ext[0]-1; i++) {
					...
					for (idx = 0; idx < dim; idx++) {
						...
						mom_for_3dnode_new[k][j][ip][idx] += dmom;
					}
				}
			}
		}
			...
			for (j = 1; j < nnode_ext[1]-1; j++) {
				for (i = 1; i < nnode_ext[0]-1; i++) {
					...
					for (idx = 0; idx < dim; idx++) {
						dmom = frac * mom_for_node[k][j][i][idx];
						mom_for_3dnode_new[k][j ][i][idx] -= dmom;
						mom_for_3dnode_new[k][jp][i][idx] += dmom;
					}
				}
			}
		}
		...
		for (k = 1; k < nnode_ext[2]-1; k++) {
			for (j = 1; j < nnode_ext[1]-1; j++) {
				for (i = 1; i < nnode_ext[0]-1; i++) {
					...
					if (dist > 0.0) {
						kp = k + 1;  // the node to whcih the momemten to be added
					}
					else {
						kp = k - 1;  // the node to whcih the momemten to be added
					}
					for (idx = 0; idx < dim; idx++) {
						dmom = frac * mom_for_node[k][j][i][idx];
						mom_for_3dnode_new[k ][j][i][idx] -= dmom;
						mom_for_3dnode_new[kp][j][i][idx] += dmom;
					}
				}
			}
		}
	...
	ncell_nmat[2] = ncell_ext[0];
	ncell_nmat[3] = nmat;
	sprintf(dataset_name, "before_put_cell:pres_3dmat(dir=%d)", dir);
	h5_write_4d(debug_gid, dataset_name, pres_3dmat, ncell_nmat);
	sprintf(dataset_name, "before_put_cell:ei_3dmat(dir=%d)", dir);
	h5_write_4d(debug_gid, dataset_name, ei_3dmat, ncell_nmat);
	sprintf(dataset_name, "before_put_cell:vf_3dmat(dir=%d)", dir);
		...
		for (j = nbdry; j <= ncell_bdry[1]; j++) {
			for (i = nbdry; i <= ncell_bdry[0]; i++) {
				for (m = 0; m < nmat; m++) {
					...
					if (if_fixed_state_ea_mat[m]) {
						rho_3dmat[k][j][i][m]   = rho_fixed_state_ea_mat[m];
						ei_3dmat[k][j][i][m]    = ei_fixed_state_ea_mat[m];
						pres_3dmat[k][j][i][m]  =  pres_fixed_state_ea_mat[m]; 
					}
					else { 
						...
						if (!solid_num_ea_mat[m]) {
							...
							p_solid(solid_num_ea_mat[m], rho_3dmat[k][j][i][m], ei_3dmat[k][j][i][m], &(pres_3dmat[k][j][i][m]));
						}
					}
					ei_3dmat[k][j][i][m] = MAX(ei_3dmat[k][j][i][m], 0.0);
					pres_3dmat[k][j][i][m] = MAX(pres_3dmat[k][j][i][m], 0.0);
				}
			}
		}  
	}
...
#ifdef H5DUMP_DEBUG
	sprintf(dataset_name, "end_of_advec:pres_3dmat(dir=%d)", dir);
	h5_write_4d(debug_gid, dataset_name, pres_3dmat, ncell_nmat);
	sprintf(dataset_name, "end_of_advec:ei_3dmat(dir=%d)", dir);
	h5_write_4d(debug_gid, dataset_name, ei_3dmat, ncell_nmat);
	sprintf(dataset_name, "end_of_advec:rho_3dmat(dir=%d)", dir);
	h5_write_4d(debug_gid, dataset_name, rho_3dmat, ncell_nmat);
	sprintf(dataset_name, "end_of_advec:vf_3dmat(dir=%d)", dir);
	h5_write_4d(debug_gid, dataset_name, vf_3dmat, ncell_nmat);
	sprintf(dataset_name, "end_of_advec:pres_3dcell(dir=%d)", dir);
	h5_write_3d(debug_gid, dataset_name, pres_3dcell, dims);
	sprintf(dataset_name, "end_of_advec:ei_3dcell(dir=%d)", dir);
	h5_write_3d(debug_gid, dataset_name, ei_3dcell, dims);
	sprintf(dataset_name, "end_of_advec:rho_3dcell(dir=%d)", dir);
	h5_write_3d(debug_gid, dataset_name, rho_3dcell, dims);
#endif
...
#ifdef H5DUMP_DEBUG
    sprintf(dataset_name, "end_of_advec:vel_3dnode(dir=%d)", dir);
    dims[0] = nnode_ext[2];
    dims[1] = nnode_ext[1];
    dims[2] = nnode_ext[0];
    dims[3] = dim;
