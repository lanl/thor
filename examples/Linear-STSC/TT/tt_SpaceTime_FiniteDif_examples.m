...
b = 1;
tol = 1e-10;
savefilename = sprintf('../plot_data/tt_ST_FD_%s.mat', testname);
%% now the time step is the same as space step
Ns = [10,50,100,150,200,250,300,350];  %[4,6,8];
R = cell(numel(Ns),1);
for i = 1:numel(Ns)
  tic
  n = Ns(i);
  ...
  %%
  [ttsol,Ctt] = tt_ST_FD_FD_3D_Solver(testcase,TN,SN,tol,a,b);
  tt_time = toc;
  %% check the error
  ...
  %% collect results
  c.sol = ttsol; %solution
  c.err = Err; % error
  c.time = tt_time; % elapsed_time
  R{i} = c;
