...
%% What to plot
itest = 2; % dataset to plot
plotacc = 0; % plot accuracy figure
plotMC = 0; % plot MC
plotadapt = 1; %plot adaptivity for ANOVA
...
if plotacc
  ...
  for i = 1:numel(TrapDs)
    ...
    label{1,i} = sprintf('d=%d, slope=%.2f',TrapDs(i),temp(1));
    plot(TrapNs,TrapErr(i,:),'-^');
  ...
  for i = 1:numel(SimpDs)
    ...
    label{1,i} = sprintf('d=%d, slope=%.2f', SimpDs(i),temp(1));
    plot(SimpNs, SimpErr(i,:),'-^');
    %   plot(log(TrapNs),log(TrapErr(i,:)));
  end
  set(gca,'XScale','log','YSCale','log');
  legend(label);
