...
Hfpos = ttt(tempXpos_noBC,tempmupos);
...
Hfposmat = reshape(permute(Hfpos,[1,3,2,4]),[nx*nmu,nx*nmu]);

tempXneg = permute(tempXpos,[2,1]);
tempXneg_noBC = tempXneg;
tempXneg_noBC(end,:) = 0;

Hfneg = ttt(tempXneg_noBC, flipud(tempmupos));
% Hfneg = ttt(tempXneg_noBC, tensor([0,0;wgt(1),0]))+ ...
%   ttt(tempXneg, tensor([0,0;0,wgt(2)]));

Hfnegmat = reshape(permute(Hfneg,[1,3,2,4]),[nx*nmu,nx*nmu]);

%%
Hf = ttt(Hfpos + Hfneg, tensor(sigma_s/2));
...
%%
Hfmatref = importdata('./Data/SigmaS_matrix_8groups.txt');
Hmatref =  importdata('./Data/Href_matrix_8groups.txt');
% Hlhsmat = Hmat-Hfmat*2;
Qvec = Q(:);
...
fprintf('compress ratio in Hs = %.2e \n', compress_ratio_tt(tt_tensor(double(Hf))));
