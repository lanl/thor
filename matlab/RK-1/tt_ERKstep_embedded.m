function [y,yerr] = tt_ERKstep_embedded(fcn, y0, t0, dt, B,ett)
%[y,yerr] = tt_ERKstep_embedded(fcn, y0, t0, dt, B,ett)
% Inputs:
%    fcn = ODE RHS function in TT format, f(t,y)
%    y0  = solution at beginning of time step in TT format
%    t0  = 'time' at beginning of time step
%    dt  = step size to take
%    B   = Butcher table to use
%   ett  = TT rounding tolerance 
% Outputs:
%    y     = new solution at t0+dt in TT format
%    yerr  = error estimate in TT format
...
for stage=1:s
  ...
  z = y0;
  for j=1:stage-1
     z = round(z + dt*A(stage,j)*k{j},ett);
  end

  % construct new stage RHS
  % Assume truncation happens in fcn 
  k{stage} = fcn(t0+dt*c(stage),z,ett);
