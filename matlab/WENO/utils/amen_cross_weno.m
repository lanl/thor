function [y,Y]=amen_cross_weno(inp, fun, tol, varargin)
% Block cross with error-based enrichment.
% function [y]=amen_cross_weno(inp, fun, tol, varargin)
...
%         end;
    ...
    if ((i==d+1)||(i==0))
        if (verb>0)
            fprintf('=amen_cross_weno= swp=%d, max_dx=%3.3e, max_rank=%d, #ifun_evals=%d, #ffun_evals=%d\n', swp, max_dx, max(ry), ievalcnt, fevalcnt);
        end;
...
%         if ((max_dx<tol_exit)||(swp==nswp)||(err_raise_cnt>=max_err_jumps))        
        if (max_dx<tol_exit)||(swp==nswp-1)||(err_raise_cnt>=max_err_jumps)
            ...
            if (err_raise_cnt>=max_err_jumps && verb > 0)
                fprintf('=amen_cross_weno= Will exit since the error stopped decreasing\n');
            end;
%             if (dir==exitdir)
%                 break;
%             else
