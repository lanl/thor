function [tt,rm]=qr_l(tt,op,core_idx)
...
if ( strcmp(op,'lr') || strcmp(op,'LR')) 
  ...
  dl = core_idx+1;
  core0=cr(pos1:pos1-1+r(dl)*n(dl)*r(dl+1));
  core0=reshape(core0,[r(dl)*n(dl),r(dl+1)]);
  [core0,rm]=qr(core0,0);
  r(dl+1)=size(core0,2);
  cr(pos1:pos1+r(dl)*n(dl)*r(dl+1)-1)=core0(:);
 % pos1=pos1+r(dl)*n(dl)*r(dl+1)-1;
